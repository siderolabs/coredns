...
// WithPositiveTTL configures the TTL for positive responses.
func WithPositiveTTL(pttl, minpttl time.Duration) func(*Cache) {
	switch {
	case pttl <= 0 || minpttl <= 0:
		panic("TTL must be greater than 0")
	case pttl < minpttl:
		panic("TTL must be greater than or equal to minTTL")
	case pttl > maxTTL || minpttl > maxTTL:
		panic(fmt.Errorf("TTL must be less than %s", maxTTL))
	case minpttl < minTTL || pttl < minTTL:
		panic(fmt.Errorf("TTL must be greater than or equal to %s", minTTL))
	}
...
// WithNegativeTTL configures the TTL for negative responses.
func WithNegativeTTL(nttl, minnttl time.Duration) func(*Cache) {
	switch {
	case nttl <= 0 || minnttl <= 0:
		panic("TTL must be greater than 0")
	case nttl < minnttl:
		panic("TTL must be greater than or equal to minTTL")
	case nttl > maxNTTL || minnttl > maxNTTL:
		panic(fmt.Errorf("TTL must be less than %s", maxNTTL))
	case minnttl < minTTL || nttl < minTTL:
		panic(fmt.Errorf("TTL must be greater than or equal to %s", minTTL))
	}
